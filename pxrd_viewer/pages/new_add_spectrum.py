...
def add_spectrum_page():
    with menutheme('Add a New Spectrum'):
        ...
        with ui.card().classes('w-full max-w-xl'):
            spectrum_name = ui.input(
                'Spectrum name (unique, no spaces)'
            ).classes('w-full').props(
                'required :rules="['
                'val => val.length <= 50 || \'Please use maximum 50 characters\','
                'val => /^[^<>:\\"/\\\\\\\\|?*\\\\s]+$/.test(val) || \'Name must not contain < > : \\" / \\\\ | ? * or spaces\','
                ']"'#< > : " / \\\\ | ? *
            )
            ...
            uploaded_file = ui.upload(label='Choose a spectrum file', multiple=False, on_upload=handle_upload,auto_upload=True,max_total_size=5*1024*1024).props('accept=.xyd').classes('w-full')
