...
from data_sources import Spectrum, delete_spectrum, edit_spectrum, list_available_spectra, ALL_ELEMENTS, list_used_tags
import altui
...
def edit_spectra_page():
    with menutheme('Edit Saved Spectra'):
        ...
        selected_name = ui.select(spectrum_names, label='Select a spectrum to edit', value=spectrum_names[0],on_change=lambda e: update_selected_spectrum(next(s for s in spectra if s.name == e.value))).classes('w-full')
        ...
        async def on_delete():
            ...
            try:
                ...
                #Error deleting spectrum: cannot access local variable 'spectra' where it is not associated with a value
